...
-- El trigger de avance solo corre en INSERT o UPDATE **of status**; actualizar solo avance_pct no lo sobrescribe
-- hasta que alguien cambie el estado del proyecto.
-- Ejecuta en SQL Editor después de schema_bodega_versions.sql (tabla project_activity).
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'comment', p_comment,
      'actor_role', r,
      'actor_username', (select nullif(trim(p.username), '') from public.profiles p where p.id = auth.uid())
    )
  );
end;
$$;
