-- Quita totales Coupa («Units 2,550.00 USD») de partidas OC y evita crear proyectos desde ellos.
-- Ejecuta en Supabase → SQL Editor. Luego: NOTIFY pgrst, 'reload schema';

create or replace function public.bodega_is_cotizacion_amount_summary_line(p_line text)
returns boolean
language sql
immutable
as $$
  select coalesce(
    trim(p_line) ~* '^Units\s+[\d,. ]+\s*(USD|EUR|MXN|CAD)$'
    or trim(p_line) ~* '^[\d,. ]+\s*(USD|EUR|MXN|CAD)$'
    or trim(p_line) ~* '^(Total|Subtotal|Grand total|Net total)\b',
    false
  );
$$;

-- Limpia cotizacion_lineas en OCs ya subidas.
update public.bodega_ordenes_compra o
set cotizacion_lineas = coalesce(
  (
    select array_agg(trim(v) order by ord)
    from unnest(o.cotizacion_lineas) with ordinality as t(v, ord)
    where trim(v) <> ''
      and not public.bodega_is_cotizacion_amount_summary_line(v)
  ),
  '{}'
)
where exists (
  select 1
  from unnest(o.cotizacion_lineas) v
  where public.bodega_is_cotizacion_amount_summary_line(v)
);

-- Elimina proyectos creados por error desde el total de la OC (solo nombre = total).
delete from public.bodega_projects p
where public.bodega_is_cotizacion_amount_summary_line(p.nombre);
...
begin
  ...
  for i in 1..n loop
    ...
    if public.bodega_is_cotizacion_amount_summary_line(line) then
      skipped := skipped + 1;
      continue;
    end if;
...
grant execute on function public.bodega_is_cotizacion_amount_summary_line(text) to authenticated;
